...
describe('CrossAccountAccessStack', () => {
  ...
  describe('IAM Role', () => {
    test('should create cross-account IAM role', () => {
      ...
      template.hasResourceProperties('AWS::IAM::Role', {
        Description: 'Cross-account role for S3 access with external ID rotation',
  ...
  describe('Lambda Function', () => {
    test('should create rotation Lambda function', () => {
      ...
      const rotationFunction = lambdaFunctions.find((func: any) => 
        func.Properties.Timeout === 300
      ) as any;
      
      expect(rotationFunction).toBeDefined();
      expect(['nodejs20.x', 'python3.11']).toContain(rotationFunction.Properties.Runtime);
      expect(['externalIdRotation.handler', 'index.lambda_handler']).toContain(rotationFunction.Properties.Handler);
    });
  ...
  describe('Input Validation', () => {
    ...
    test('should throw error for empty principal ARNs', () => {
      ...
      }).toThrow('externalPrincipalArns must contain at least one principal ARN');
    });
    ...
    test('should throw error for invalid principal ARN', () => {
      ...
      }).toThrow('externalPrincipalArns[0] must be a valid IAM ARN from the external account');
    });
